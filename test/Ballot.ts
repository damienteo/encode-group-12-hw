...
describe("Ballot", () => {
  ...
  describe("when the chairperson interacts with the giveRightToVote function in the contract", () => {
    it("gives right to vote for another address", async () => {
      const giveRightTx = await ballotContract.giveRightToVote(
        accounts[2].address
      );
    ...
    it("can not give right to vote for someone that has voted", async () => {
      ...
      await voteTx.wait();
      await expect(
        ballotContract.giveRightToVote(deployer.address)
    ...
    it("can not give right to vote for someone that has already voting rights", async () => {
      ...
      await expect(
        ballotContract.giveRightToVote(accounts[3].address)
  ...
  describe("when the voter interact with the delegate function in the contract", () => {
    it("should transfer voting power", async () => {
      ...
      const { voted, delegate, vote } = await ballotContract.voters(
        accounts[1].address
      );
  ...
  describe("when the an attacker interact with the giveRightToVote function in the contract", () => {
    it("should revert", async () => {
      await expect(
        ballotContract.connect(accounts[5]).giveRightToVote(accounts[6].address)
  ...
  describe("when the an attacker interact with the vote function in the contract", () => {
    it("should revert", async () => {
      await expect(
        ballotContract.connect(accounts[5]).vote(1)
  ...
  describe("when the an attacker interact with the delegate function in the contract", () => {
    it("should revert", async () => {
      await expect(
        ballotContract.connect(accounts[5]).delegate(accounts[6].address)
  ...
  describe("when someone interact with the winningProposal function before any votes are cast", () => {
    it("should return 0", async () => {
      const winningProposal = await ballotContract
        .connect(accounts[1])
        .winningProposal();
      ...
      for await (const index of Array.from(Array(5).keys())) {
        ...
        await (
          await ballotContract
            .connect(account)
            .vote(generateRandom(0, PROPOSALS.length))
        ).wait();
      }
      const winningProposal = await ballotContract.winningProposal();
      const proposal = await ballotContract.proposals(winningProposal);
      expect(ethers.utils.parseBytes32String(proposal.name)).to.eq(
        PROPOSALS[winningProposal.toNumber()]
      );
    });
  });
});
